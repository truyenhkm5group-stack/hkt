...
export function testMarketerAttributionGoesByKey() {
  ...
  */
  ...
  assert.equal(MARKETER_LINK_STATES.length, 7, "bảy tình trạng quy kết");
  assert.deepEqual([...MARKETER_RESOLVED_STATES], ["RESOLVED", "RESOLVED_BY_PAGE"], "đúng hai tình trạng được ghi tên một người");
  ...
  */
  ...
  const viTriMarketer = src.indexOf("export const OM_MARKETER_ID");
  const bieuThuc = src.slice(viTriMarketer, src.indexOf("export const OM_EVIDENCE"));
  const thuTuTrongSql = MARKETER_EVIDENCE_ORDER.map((e) => (e === "AD_CAMPAIGN" ? bieuThuc.indexOf("om_marketer.marketer_id") : bieuThuc.indexOf("om_fp.marketer_id")));
  assert.ok(
    thuTuTrongSql.every((v, i) => v >= 0 && (i === 0 || v > thuTuTrongSql[i - 1])),
    "coalesce của OM_MARKETER_ID phải xếp theo đúng MARKETER_EVIDENCE_ORDER — thứ tự thẩm quyền chỉ được khai một chỗ",
  );
  ...
  console.log("✓ Quy kết marketer: HAI đường đi bằng KHOÁ (chiến dịch → ảnh chụp fanpage), 0 phép dò chữ · thứ tự thẩm quyền khai một chỗ · 7 tình trạng, 5 loại chỗ trống đều có việc phải làm");
}
...
export function testEvidenceSplitIsAPartition(report: {
  ...
  console.log(`✓ Hai đường quy kết là một phép CHIA: Σ bằng chứng = ${report.marketerCoverage.resolved} quy kết được · Σ tình trạng = ${tongTheoTinhTrang} ca · ${report.marketerCoverage.conflicts} đơn mâu thuẫn`);
}
...
async function dungDuLieu(db: Db) {
  ...
  */
  const anhChup = [
    { id: `${P}o1`, marketerId: `${P}mk`, status: "ATTRIBUTED" },
    { id: `${P}o2`, marketerId: `${P}mk`, status: "ATTRIBUTED" },
    { id: `${P}o3`, marketerId: null, status: "NO_ASSIGNMENT" },
  ];
  for (const a of anhChup) {
    await db
      ...
      .values({
        ...
        ruleVersion: 1,
        attributionSource: "PANCAKE_PAGE",
...
export async function testReasonDenominatorsAndFilters(db: Db) {
  ...
  */
  ...
  assert.equal(theoFanpage.byEvidence.FANPAGE_ASSIGNMENT, 2, "cả hai đi bằng đường FANPAGE — bộ dữ liệu này không có chiến dịch nào");
  assert.equal(theoFanpage.byEvidence.AD_CAMPAIGN, 0, "không đơn nào đi bằng đường quảng cáo ở đây — nếu khác 0 thì hai đường đang bị gộp");
