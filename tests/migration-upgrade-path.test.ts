...
export async function testMigrationUpgradePath() {
  ...
  try {
    ...
    */
    ...
    /*
      Company OS · Agent G (0133). Một dòng nhật ký và một yêu cầu duyệt ĐANG CHỜ có từ trước — bước
    ...
    */
    ...
    // 0133: dòng cũ ở NULL (chưa biết), ràng buộc mới chặn giá trị lạ, chỉ mục chống trùng chờ duyệt chạy.
    assert.equal(await dem("select count(*)::int as n from audit_logs where id = 'up-al1' and actor_kind is null and correlation_id is null and reason is null"), 1, "0133: dòng nhật ký cũ phải ở NULL — KHÔNG đoán là người dùng");
    assert.equal(await dem("select count(*)::int as n from approval_requests where id = 'up-ar1' and payload_fingerprint is null and status = 'PENDING'"), 1, "0133: yêu cầu cũ giữ nguyên, không dựng dấu vân tay");
    await assert.rejects(client.query(`insert into audit_logs (id, user_email, action, entity, actor_kind) values ('up-al2', 'x', 'x', 'X', 'ROBOT')`), "0133: loại tác nhân lạ phải bị CSDL từ chối");
    ...
    await assert.rejects(
      ...
      "0133: hai yêu cầu ĐANG CHỜ cùng dấu vân tay của cùng người phải bị chặn",
    );
    ...
    */
    ...
    assert.ok(!/update/i.test(readFileSync(path.join(goc, "0134_company_os_economics.sql"), "utf8").replace(/--.*$/gm, "")), "0134: migration KHÔNG được chứa UPDATE — không backfill dòng sổ cũ");
    assert.equal(
