...
const MOI = [
  ...
  "0092_fb_adsets",
  "0094_payroll_policy_engine",
  "0095_payroll_run_lifecycle",
  "0096_payroll_input_approval",
] as const;
...
export async function testMigrationUpgradePath() {
  ...
  try {
    ...
    */
    ...
    assert.equal(await dem("select count(*)::int as n from information_schema.tables where table_name = 'salary_policies'"), 0, "bước 1: bảng salary_policies CHƯA được có — đó là thứ 0094 thêm vào");
    assert.equal(await dem("select count(*)::int as n from information_schema.columns where table_name = 'marketer_profit_carryover' and column_name = 'component_code'"), 0, "bước 1: cột component_code CHƯA được có — đó là thứ 0094 thêm vào");
    assert.equal(await dem("select count(*)::int as n from information_schema.columns where table_name = 'payroll_periods' and column_name = 'approved_by'"), 0, "bước 1: cột approved_by CHƯA được có — đó là thứ 0095 thêm vào");
    assert.equal(
      await dem("select count(*)::int as n from information_schema.columns where table_name = 'payroll_inputs' and column_name = 'status'"),
      0,
      "bước 1: cột trạng thái của đầu vào nhập tay CHƯA được có — đó là thứ 0096 thêm vào",
    );
    ...
    */
    ...
    /*
      ═══ 0094 · CHÍNH SÁCH LƯƠNG CHUNG CHO TOÀN CÔNG TY ═══
    ...
    */
    for (const bang of ["salary_policies", "salary_policy_versions", "salary_policy_components", "employment_assignments", "employee_policy_assignments", "payroll_inputs", "payroll_adjustments"]) {
      assert.equal(await dem(`select count(*)::int as n from information_schema.tables where table_name = '${bang}'`), 1, `0094: bảng ${bang} phải được tạo`);
      assert.equal(await dem(`select count(*)::int as n from ${bang}`), 0, `0094: ${bang} phải RỖNG — nâng cấp không được khai hộ ai một cơ chế trả tiền`);
    }
    ...
    */
    await assert.rejects(
      ...
      "0094: bật bù lỗ trên doanh thu bị chặn — doanh thu không bao giờ âm",
    );
    await assert.rejects(
      ...
      "0094: trần thấp hơn sàn bị chặn — cặp ấy không có giá trị nào thoả",
    );
    ...
    await assert.rejects(
      ...
      (e: unknown) => /unique|duplicate/i.test(String((e as { message?: string })?.message ?? e)),
      "0094: hai thành phần cùng khoá trong một phiên bản bị chặn — máy tính gộp theo khoá nên chúng sẽ cộng thành một dòng không ai đối chiếu lại được",
    );
    ...
    */
    ...
    await assert.rejects(
      ...
      "0094: xoá chính sách đang gán cho người bị chặn — lịch sử lương tính bằng nó sẽ mồ côi",
    );
    ...
    await assert.rejects(
      ...
      "0094: mốc kết thúc trước mốc bắt đầu bị chặn",
    );
    await assert.rejects(
      ...
      "0094: nơi làm việc lạ bị chặn ở CSDL — danh sách ĐÓNG, không có ô gõ tự do",
    );
    ...
    */
    ...
    await assert.rejects(
      ...
      (e: unknown) => /unique|duplicate/i.test(String((e as { message?: string })?.message ?? e)),
      "0094: nhập lại cùng đại lượng cho cùng kỳ phải là SỬA — dòng thứ hai làm mỗi lượt tính lại cộng dồn",
    );
    ...
    */
    await assert.rejects(
      ...
      "0094: số tiền điều chỉnh âm bị chặn — dấu do loại khoản quyết định, không do người gõ",
    );
    await assert.rejects(
      ...
      "0094: loại khoản lạ bị chặn — danh sách ĐÓNG",
    );
    await assert.rejects(
      ...
      "0094: khoản tiền không có LÝ DO bị chặn — một khoản không ai duyệt lại được",
    );
    ...
    /*
      ...
      nâng cấp.
    */
    assert.equal(await dem("select count(*)::int as n from marketer_profit_carryover where component_code = 'MARKETING_PROFIT'"), 2, "0094: dòng sổ lỗ có từ trước phải mang khoá thành phần của đường tính cũ");
    ...
    assert.equal(await dem("select count(*)::int as n from marketer_profit_carryover where employee_id = 'mkt-1' and month_key = '2026-09'"), 2, "0094: hai khoản bù lỗ của cùng một người là hai chuỗi số dư RIÊNG");
    await assert.rejects(
      ...
      (e: unknown) => /unique|duplicate/i.test(String((e as { message?: string })?.message ?? e)),
      "0094: nới khoá KHÔNG phải nới lỏng — một người + một tháng + một thành phần vẫn chỉ MỘT dòng",
    );

    /*
      ═══ 0095 · VÒNG ĐỜI KỲ LƯƠNG: SÁU TRẠNG THÁI, VÀ `FINAL` CŨ KHÔNG BỊ VIẾT LẠI ═══
    ...
    */
    assert.equal(await dem("select count(*)::int as n from information_schema.columns where table_name = 'payroll_periods' and column_name = 'approved_by'"), 1, "0095: cột người duyệt phải được thêm");
    assert.equal(await dem("select count(*)::int as n from payroll_periods where status = 'FINAL'"), 1, "0095: dòng FINAL có từ trước KHÔNG bị viết lại — đó là một kỳ đã trả tiền");
    ...
    assert.equal(await dem("select count(*)::int as n from payroll_periods where status = 'CALCULATED'"), 1, "0095: trạng thái CALCULATED ghi được");
    ...
    */
    await assert.rejects(
      ...
      "0095: duyệt mà không có mốc duyệt bị chặn",
    );
    ...
    await assert.rejects(
      ...
      "0095: khoá mà không có mốc khoá bị chặn",
    );
    ...
    await assert.rejects(
      ...
      "0095: đánh dấu đã trả mà không có mốc trả bị chặn",
    );
    await assert.rejects(
    ...
    /*
      ═══ 0096 · ĐẦU VÀO NHẬP TAY: ĐƠN VỊ · TRẠNG THÁI · NGƯỜI DUYỆT ═══
    ...
    */
    ...
    assert.equal(await dem("select count(*)::int as n from payroll_inputs where id = 'up-pi3' and status = 'ENTERED'"), 1, "0096: dòng mới mặc định CHỜ DUYỆT — không tự coi một con số vừa gõ là đã có người soát");
    assert.equal(await dem("select count(*)::int as n from payroll_inputs where id = 'up-pi3' and unit = ''"), 1, "0096: KHÔNG backfill đơn vị — đoán đơn vị của một con số đã nhập là đoán ý người nhập");
    ...
    await assert.rejects(
      ...
      "0096: đánh dấu đã duyệt mà không có mốc duyệt bị chặn — một chữ ký không có ngày là một chữ ký trống",
    );
    await assert.rejects(
    ...
    assert.equal(await dem("select count(*)::int as n from payroll_inputs where id = 'up-pi3' and status = 'APPROVED' and approved_at is not null"), 1, "0096: duyệt kèm mốc thời gian thì ghi được");
