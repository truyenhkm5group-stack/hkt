...
export async function testMigrationUpgradePath() {
  ...
  try {
    ...
    */
    ...
    // 0141 (Company OS · Agent X): CHECK loại nhận hai loại của vòng phản hồi tồn → creative / quảng cáo,
    // loại cũ vẫn nhận, loại lạ vẫn bị chặn (0141 chỉ thay CHECK, không nới nó thành "gì cũng được").
    await client.query(`insert into recommendation_decisions (id, source_key, kind, decision, decided_by_user_id, snapshot) values ('up-rd7', 'stock:PUSH:p:x', 'STOCK_PUSH', 'ACCEPTED', 'up-uh', '{}')`);
    await client.query(`insert into recommendation_decisions (id, source_key, kind, decision, decided_by_user_id, snapshot) values ('up-rd8', 'stock:SCALE:p:x', 'SCALE_STOCK_RISK', 'ACCEPTED', 'up-uh', '{}')`);
    ...
    // 0145 (Company OS · Agent N): ba cột ghi vết gửi lại tin cảnh báo. Dòng ghi KHÔNG nhắc tới chúng — đúng như
    ...
    assert.equal(await dem("select count(*)::int as n from notifications where id = 'up-nt1' and notify_attempts is null and notify_last_attempt_at is null and notify_last_error is null"), 1, "0145: dòng cũ giữ NULL ở cả ba cột (không backfill)");
    const sql0145 = readFileSync(path.join(goc, "0145_notification_retry.sql"), "utf8").replace(/--.*$/gm, "");
    assert.ok(!/(^|;|\s)UPDATE\s+"?notifications/im.test(sql0145), "0145: migration KHÔNG được chứa UPDATE — không backfill");
    assert.ok(!/DEFAULT/i.test(sql0145), "0145: không DEFAULT — dòng mới chưa thử cũng là NULL");
