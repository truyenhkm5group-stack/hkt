...
export async function testFinanceCockpit(db: Db) {
  ...
  */
  ...
  // ═══ 3. CHUYỂN NỘI BỘ KHÔNG ĐƯỢC THỔI PHỒNG TIỀN VÀO / TIỀN RA ═══
  /*
    Hai đầu của lần chuyển 15 triệu đều nằm trong sổ. Tính chúng vào thì "tiền vào" thành 65 triệu
    và "tiền ra" thành 35 triệu — chênh lệch vẫn đúng nhưng chủ shop đọc thành "tháng này quay vòng
    65 triệu", gần gấp rưỡi thực tế.
  */
  assert.equal(bc.moneyIn, 50_000_000, "3. tiền vào = 30tr COD + 20tr vay, KHÔNG gồm 15tr chuyển nội bộ");
  assert.equal(bc.moneyOut, 20_000_000, "3. tiền ra = 8 + 5 + 1 + 6 triệu, KHÔNG gồm 15tr chuyển nội bộ");
  assert.equal(bc.net, 30_000_000, "3. và chênh lệch vẫn đúng bằng tổng phát sinh");
