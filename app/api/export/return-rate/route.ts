...
export async function GET(request: NextRequest) {
  ...
  // CÙNG bộ lọc giá trị đơn với màn hình: tệp gửi đi phải là đúng bảng người gửi đang nhìn.
  const giaTriDon = parseOrderValue(sp);
  const { all } = await getReturnRateByVariant({ period, q, minShipped, value: giaTriDon, sort: "successRate", dir: "asc", page: 1, pageSize: 1 });
  ...
  lines.push("");
  lines.push(csvCell(`Kỳ: ${period.label} · ${orderValueLabel(giaTriDon)} · tỷ lệ giao thành công = giao TC (COD thực > 100K) / (giao TC + không TC) · xuất lúc ${new Date().toISOString()}`));
