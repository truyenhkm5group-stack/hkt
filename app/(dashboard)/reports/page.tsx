...
import { parseTargetMargin } from "@/lib/constants/estimated-cost";
...
const PNL_LINES: LineDef[] = [
  {
    label: "Doanh thu đơn giao thành công",
    key: "revenue",
    kind: "revenue",
    note: "Đơn ở trạng thái Đã nhận / Đã thu tiền",
  },
  {
    label: "(–) Giá vốn hàng bán",
    key: "cogs",
    kind: "cost",
    note: "Giá vốn của các đơn giao thành công",
  },
...
export default async function ReportsPage({
  ...
  */
  ...
  // Biên LN người xem gõ để tính trần CPQC — không có mặc định (AGENTS.md mục 38).
  const bienMucTieu = parseTargetMargin(param(raw, "bien"));
  /*
  ...
  */
  const locQuery = `${giaTriDon.min !== null ? `&vmin=${giaTriDon.min}` : ""}${giaTriDon.max !== null ? `&vmax=${giaTriDon.max}` : ""}${tinhCPQC ? "" : "&ads=0"}${bienMucTieu === null ? "" : `&bien=${bienMucTieu}`}`;
  ...
  const grossMargin = current.revenue
  ...
  return (
    <div className="space-y-5">
      ...
      <p className="-mt-3 text-[11.5px] text-muted-foreground">
        ...
        <NavLink href="/reports/target" className="font-medium text-primary hover:underline">Kế hoạch mục tiêu lợi nhuận</NavLink>
      </p>
        ...
        <>
            ...
            facets={[
              {
                ...
                label: "Cơ sở tính",
                options: [
                  { value: "created", label: REPORT_BASIS_LABEL.created },
                  { value: "delivered", label: REPORT_BASIS_LABEL.delivered },
                ],
                single: true,
              },
            ]}
          ...
          <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <MetricCard
              ...
              value={formatVND(current.revenue, { compact: true })}
              change={change(current.revenue, previous?.revenue)}
              note={`${formatNumber(current.successOrders)} đơn · TB ${formatVND(current.successOrders ? Math.round(current.revenue / current.successOrders) : 0, { compact: true })}/đơn`}
              icon={PackageCheck}
              tone="blue"
            />
            <MetricCard
              label="Lãi gộp"
              value={formatVND(current.grossProfit, { compact: true })}
              change={change(current.grossProfit, previous?.grossProfit)}
              note={`Biên gộp ${grossMargin.toFixed(1)}% · giá vốn ${formatVND(current.cogs, { compact: true })}`}
              icon={Boxes}
              tone="green"
            />
            <MetricCard
          ...
          <section className="grid gap-5 xl:grid-cols-[minmax(0,1.7fr)_minmax(300px,0.8fr)]">
            ...
            >
                    ...
                    {PNL_LINES.map((line) => {
                      ...
                      return (
                        <TableRow
                          key={line.key}
                          className={cn(
                            emphasis && "bg-muted/40 hover:bg-muted/40",
                        ...
                        >
                          ...
                          <TableCell className="text-right text-xs text-muted-foreground">
                            {current.revenue
                              ? `${((value / current.revenue) * 100).toFixed(1)}%`
                              : "—"}
                          </TableCell>
                          <TableCell className="text-right text-xs text-muted-foreground">
                            {previous ? formatVND(prev ?? 0) : "—"}
                          </TableCell>
                          <TableCell className="text-right">
                            <Delta
                              current={value}
                              previous={prev}
                              invert={line.kind === "cost"}
                            />
                          </TableCell>
                        </TableRow>
                      );
              ...
              <div className="flex flex-wrap gap-x-6 gap-y-1 border-t px-5 py-3 text-xs text-muted-foreground">
                ...
                <span>
                  Lợi nhuận / đơn giao thành công:{" "}
                  <strong className="text-foreground">
                    {formatVND(
                      current.successOrders
                        ? Math.round(current.netProfit / current.successOrders)
                        : 0,
                    )}
                  </strong>
                </span>
                {current.adSpend ? (
                  <span>
                    ...
                    <strong className="text-foreground">
                      {current.adOrders
                        ? formatVND(
                            Math.round(current.adSpend / current.adOrders),
                          )
                        : "—"}
                    </strong>
                  </span>
              ...
              >
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-lg border bg-background p-3">
                    ...
                    <p className="numeric mt-1 text-xl font-bold">
                      {formatNumber(current.returned)}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      {pct(current.returned, current.orders).toFixed(1)}% đơn
                      không huỷ
                    </p>
                  </div>
                  <div className="rounded-lg border bg-background p-3">
                    ...
                    <p className="numeric mt-1 text-xl font-bold">
                      {formatNumber(current.cancelled)}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      {pct(current.cancelled, totalOrders).toFixed(1)}% tổng đơn
                      lên
                    </p>
                  </div>
                </div>
                <div className="mt-3 flex items-center justify-between rounded-lg bg-destructive/5 px-3 py-2 text-sm">
                  <span className="text-xs font-medium text-muted-foreground">
                    Phí ship + phí hoàn mất trắng
                  </span>
                  <Money
                    value={current.lostShipping}
                    className="font-bold text-destructive"
                  />
                </div>
                {previous ? (
                  <p className="mt-2 text-[11px] text-muted-foreground">
                    Kỳ trước: {formatNumber(previous.returned)} hoàn (
                    {pct(previous.returned, previous.orders).toFixed(1)}%) ·{" "}
                    {formatNumber(previous.cancelled)} huỷ · mất{" "}
                    {formatVND(previous.lostShipping, { compact: true })}
                  </p>
                ) : null}
              </SectionCard>
            </div>
          ...
          <section className="grid gap-5 xl:grid-cols-2">
            ...
            >
              <BreakdownTable
                ...
                firstHeader="Kênh"
                renderKey={(key) => <SourceBadge source={key} />}
              />
            </SectionCard>
            <SectionCard
            ...
            >
              <BreakdownTable
                rows={report.sellers}
                firstHeader="Nhân viên"
                renderKey={(key) => <span className="font-medium">{key}</span>}
              />
            </SectionCard>
          </section>
                ...
                <TableBody>
                  {report.products.length ? (
                    ...
                    <TableRow>
                      ...
                      >
                        Chưa có đơn giao thành công trong kỳ.
                      </TableCell>
                    </TableRow>
                  )}
                </TableBody>
            ...
            rows.map((r) => {
              ...
              return (
                <TableRow key={r.key}>
                  ...
                  <TableCell className="text-right">
                    <span className="numeric">{formatNumber(r.orders)}</span>
                  </TableCell>
                  <TableCell className="text-right">
                    <span className="numeric font-semibold">
                      {formatNumber(r.success)}
                    </span>
                  </TableCell>
                  <TableCell className="text-right">
                    <span
                  ...
                  <TableCell className="text-right">
                    <Money value={r.cogs} className="text-muted-foreground" />
                  </TableCell>
                  <TableCell className="text-right">
                    <Money
                      value={r.grossProfit}
