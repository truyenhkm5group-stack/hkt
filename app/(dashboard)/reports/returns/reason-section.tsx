...
import { REASON_CONFIDENCE_LABEL, RETURN_REASON_OWNER } from "@/lib/constants/return-reason";
        ...
        */}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="border-b text-[11.5px] uppercase tracking-wide text-muted-foreground">
              <tr>
                <th className="py-1.5 text-left font-semibold">Lý do</th>
                <th className="py-1.5 text-right font-semibold">Số đơn</th>
                <th className="py-1.5 text-right font-semibold">Tỷ trọng</th>
                <th className="py-1.5 text-left font-semibold">Chứng cứ</th>
                <th className="py-1.5 text-left font-semibold">Sửa ở đâu</th>
              </tr>
            </thead>
            <tbody className="divide-y">
              {bc.reasons.map((r) => (
                <tr key={r.reason} className={r.reason === "UNKNOWN" ? "text-muted-foreground" : ""}>
                  <td className="py-1.5">{r.label}</td>
                  <td className="py-1.5 text-right tabular-nums">{formatNumber(r.count)}</td>
                  <td className="py-1.5 text-right tabular-nums">{formatPercent(r.share)}</td>
                  <td className="py-1.5 text-[11px] text-muted-foreground">
                    {(["CONFIRMED", "CARRIER_CODE", "CARRIER_TEXT"] as const)
                      .filter((c) => r.confidence[c] > 0)
                      .map((c) => `${REASON_CONFIDENCE_LABEL[c]} ${r.confidence[c]}`)
                      .join(" · ") || "—"}
                  </td>
                  <td className="py-1.5 text-[11px] text-muted-foreground">{r.reason === "UNKNOWN" ? "—" : RETURN_REASON_OWNER[r.reason]}</td>
                </tr>
              ))}
            </tbody>
          </table>
