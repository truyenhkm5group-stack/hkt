  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        hint="Nếu tỷ lệ giao thành công tăng, giá vốn tăng, cước đổi — lợi nhuận đi về đâu. Điểm xuất phát lấy nguyên từ báo cáo lợi nhuận của kỳ đang xem, cùng công thức và cùng ORDER_OUTCOME — không con số nào ở đây được gõ tay. Mô hình tuyến tính quanh điểm hiện tại: dùng để so sánh phương án, không dùng làm dự báo."
      />

      <SectionCard title="Đặt kịch bản" description={`Kỳ đang xem: ${period.label}`} hint="Để trống là giữ nguyên hiện tại.">
              ...
              return (
                <label key={k} className="block space-y-1">
                  <span className="flex items-center gap-1 text-[13px] font-medium">
                    {SCENARIO_LEVER_LABEL[k]}
                    <InfoHint>{SCENARIO_LEVER_NOTE[k]}</InfoHint>
                  </span>
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricCard
          ...
          hint="Kịch bản trừ hiện tại · dương là tốt hơn"
          icon={Scale}
        ...
        <MetricCard
          ...
          hint={r.breakEvenSuccessRate === null ? undefined : "Giữ nguyên các đòn bẩy khác của kịch bản, cần đạt mức này để không lỗ"}
          note={
            r.breakEvenSuccessRate === null ? (
              <DataWarnings items={["Chưa giải được: không đủ đơn kết thúc, hoặc mỗi đơn giao thêm vẫn không bù nổi chi phí"]} />
            ) : undefined
          }
          icon={Scale}
      ...
      <SectionCard
        title="Từng dòng lãi lỗ"
        description={r.adsAttributionCoverage === null ? <DataWarnings items={["Độ phủ quy kết quảng cáo: chưa đo được"]} /> : undefined}
        hint={
          <>
            <p className="mb-2">Hiện tại so với kịch bản.</p>
            <p className="mb-1 font-medium">Đọc trước khi tin con số — giả định của mô hình:</p>
            <p className="mb-1">
