...
export default async function NewProductionOrderPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return (
    <div className="space-y-5">
      <PageHeader eyebrow="Kho" title={`Bảng chốt đặt hàng · ${m.product.code ? `${m.product.code} · ` : ""}${m.product.name}`} description="Số lượng khởi tạo theo đề xuất của ERP — sửa rồi bấm Chốt."
 hint="Số lượng khởi tạo theo đề xuất của ERP (tồn khả dụng, tốc độ bán, thời gian sản xuất). Sửa từng ô rồi bấm Chốt để lưu và in / gửi xưởng." />
