...
import { classifyBankTransactions, updateBankAccount } from "@/lib/actions/bank";
...
export function ConfirmInternalTransferButton({ outId, inId }: { outId: string; inId: string }) {
  ...
  return (
    <Button
      ...
      onClick={() =>
        startTransition(async () => {
          const res = await classifyBankTransactions({ ids: [outId, inId], group: "INTERNAL_TRANSFER" });
          if ("error" in res) {
            toast.error(res.error);
            return;
          }
          toast.success("Đã gán chuyển nội bộ cho cả hai dòng");
          router.refresh();
        })
      }
    >
