 ...
 *
 * "Duyệt mẫu" (trước 26/09/2026 tên "Duyệt lô") đứng đầu: đó là việc DUY NHẤT của vòng mẫu có hạn chót, và quá hạn là cả ngày test mất.
...
export function CreativeTabs({ active, defaultTab = "nguon", pendingApproval = false }: { active: string; defaultTab?: string; pendingApproval?: boolean }) {
  ...
  const [, setState] = useQueryStates(
    { tab: parseAsString, page: parseAsString, q: parseAsString, loai: parseAsString, bat: parseAsString, lo: parseAsString, ngay: parseAsString },
    { shallow: false, history: "push", startTransition },
  );
  return (
    <Tabs value={active} onValueChange={(value) => void setState({ tab: value === defaultTab ? null : value, page: null, q: null, loai: null, bat: null, lo: null, ngay: null })}>
