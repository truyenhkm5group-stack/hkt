...
import { MANUAL_GEN_IMAGE_STATUSES, MANUAL_GEN_IMAGE_STATUS_LABEL, MANUAL_GEN_KIND_LABEL } from "@/lib/constants/creative-loop";
 ...
 *
 * Mặc định THIẾT KẾ MỚI: người chọn các mẫu bán tốt làm cảm hứng + ý tưởng → máy vẽ tới 10 thiết kế mới
 * (lai DNA) vào khu này (KHÔNG vào lô). Kiểu phụ "ảnh mới cho mẫu đang có": ảnh sản phẩm thật (+ quảng cáo
 * cũ cùng mã) → 10 ảnh của CHÍNH mẫu ấy — `?product=` (đề xuất đẩy tồn) mở thẳng kiểu này. Sau đó như nhau:
 * người Duyệt / Loại từng ảnh → máy viết câu chữ theo ảnh → người soạn tên + câu chữ rồi "Đưa vào lô" chờ
 * duyệt đăng (thiết kế được cấp mã TK lúc ấy). Trần chi ảnh là trần chung với lô hằng ngày.
 */
      ...
      <div className="space-y-4">
        ...
        {canEdit ? (
          <ManualGenForm
            ...
            sources={p.sources}
            allowedNow={p.capacity.allowedNow}
            capReason={p.capacity.reason}
            disabledReason={null}
            initialPhotoId={chon.photoId}
          />
        ) : null}
