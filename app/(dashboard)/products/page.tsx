import { AlertTriangle, Boxes, Download, PackageX, ShoppingBag, Warehouse } from "lucide-react";
...
export default async function ProductsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const params = parseListParams(raw, { defaultSort: "sku", defaultDir: "asc", filterKeys: ["stock", "category", "warehouse", "status"], sortable: PRODUCT_SORTABLE, defaultPeriod: "all" });
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <MetricCard label="Sắp hết hàng" value={formatNumber(summary.low)} note="Tồn khả dụng từ 1 đến 5" icon={AlertTriangle} tone="amber" />
        <MetricCard label="Hết hàng" value={formatNumber(summary.out)} note="Tồn khả dụng ≤ 0, vẫn đang bán" icon={PackageX} tone={summary.out > 0 ? "rose" : "slate"} />
        ...
        <MetricCard label="Đã bán 30 ngày" value={formatNumber(summary.sold30)} note="Số sản phẩm, không tính đơn huỷ" icon={ShoppingBag} tone="green" />
      </section>
