...
Pancake POS ──(API + webhook)──▶  ERP  ◀──(API + webhook)── Viettel Post
   ...
   Khách hàng · Đổi/trả · Chi phí vận hành · Quảng cáo · Báo cáo lợi nhuận · Kết nối dữ liệu · Người dùng · Nhật ký
```
...
| **Ghi nhận đơn theo fanpage & % LN theo mã** | Đơn, doanh thu, lợi nhuận của mỗi mã ghi nhận cho marketer theo **fanpage phát sinh đơn**, theo thứ tự căn cứ: **ảnh chụp người phụ trách page TẠI MỐC ĐƠN LÊN** (`order_attributions`, khai ở Marketing → Fanpage & quy kết — đổi người phụ trách hôm nay KHÔNG làm đổi số của kỳ đã qua) → bảng gán phẳng ở Lương & hoa hồng (lấp chỗ, không có mốc hiệu lực) → `ad_id` của đơn (lấp chỗ; **quảng cáo không ghi đè người được tính đơn theo fanpage**) → chia theo tỷ trọng QC → về chủ mã. Trang Lương hiện độ phủ của bốn nhóm ấy, cộng lại bằng tổng đem chia. Mỗi mã khai chủ mã + X% (chủ mã hưởng từ đơn của mình, còn lại shop giữ) + Y% (người chạy cùng hưởng từ đơn mình tạo, 100 − Y về chủ mã); LN âm người tạo đơn chịu. Áp dụng cho LN1/LN2 và LN danh nghĩa theo marketer (bấm tên để xem chi tiết từng mã). |
| **Bảng dạng cây cha – con** | Bảng dùng chung (`DataTable` prop `group`) gom mẫu mã theo mã hàng với dòng cha tổng hợp, mũi tên xổ / thu gọn từng nhóm hoặc tất cả: áp dụng cho **Tỷ lệ hoàn theo mã hàng** (tổng đã gửi / giao / hoàn / tỷ lệ theo mã) và **Sản phẩm & tồn kho** (tổng tồn từng kho, đã bán, giá trị tồn theo mã). Kế hoạch đặt hàng SX và LN theo marketer đã có breakdown riêng. |
...
| **Đơn landing page** | Khách điền form → Google Sheet → ERP đọc CSV export gần như tức thời — job nhanh `landing-sheet?new=1` nạp dòng mới mỗi phút, job đầy đủ (ghép lại theo SĐT, cập nhật dòng đã sửa) mỗi 10 phút; trang tự làm mới 30 giây/lần (sheet chia sẻ “Bất kỳ ai có liên kết – Người xem”): tự dò cột theo tiêu đề (ghi đè được), cột dò được trống thì quét cả dòng để lấy size / màu, gói mua, địa chỉ (form đổi bố cục giữa các đợt); không đoán mẫu mã khi không rõ size lẫn màu; 1 sản phẩm không ghi giá = giá mặc định 499k + phí ship, gói ≥ 2 sản phẩm lấy giá gói & free ship; dòng thiếu địa chỉ / size **báo đỏ** để chăm sóc (lọc “Thiếu địa chỉ / size”); khoá theo dòng nên không nhập trùng; ghép mẫu mã Pancake theo mã hàng / tên / size / màu (chọn tay nếu chưa ghép); **cảnh báo trùng SĐT** (landing khác hoặc đơn Pancake trong N ngày) và **rủi ro hoàn** theo lịch sử khách (cùng quy tắc đơn rủi ro); nhân viên xác nhận / huỷ, bấm **Gửi POS** tạo đơn nháp (trạng thái Mới) trên Pancake kèm ghi chú rủi ro; ERP tự ghép đơn Pancake cùng SĐT để theo dõi chưa gửi → đang giao → giao thành công / hoàn / huỷ (kết quả ưu tiên Viettel Post). Quyền `landing:view / manage / config`. |
...
| **Kết quả đơn (ORDER_OUTCOME)** | Dùng chung cho mọi báo cáo, **ưu tiên trạng thái vận đơn Viettel Post** trước trạng thái Pancake: vận đơn hoàn → hoàn; vận đơn đã giao → giao thành công (trừ quy tắc phát hiện hoàn: COD thu ≤ 100K); vận đơn đang đi → đang giao; chưa có trạng thái vận đơn mới xét theo Pancake. |
| **Lương & hoa hồng** | Lương = **lương cứng THUỘC KỲ** (khai theo tháng, chia theo số ngày chồng lấn — cùng một hàm với máy chi phí) + % lợi nhuận tổng + % lợi nhuận cá nhân + % doanh thu cá nhân; thưởng chỉ tính khi số dương. Mặc định và là cơ sở **DUY NHẤT được phép chốt lương**: **LN1** — doanh thu giao thành công − quảng cáo − giá vốn CỦA CHÍNH HÀNG ĐÃ GIAO − vận chuyển − chi phí vận hành đã ghi nhận. Ba cơ sở còn lại (LN2 theo giá vốn hàng nhập · dòng tiền thực · danh nghĩa) vẫn xem được để đối chiếu nhưng màn hình báo đỏ nếu dùng để chốt lương, kèm lý do (`PAYROLL_BASIS_ELIGIBILITY`). Trang còn hiện: **đã trả trong kỳ** (chứng từ chi nhóm "Lương", theo ngày phát sinh — chiều khác hẳn phải trả), **cảnh báo của máy chi phí** ("lợi nhuận này đã trừ đủ chi phí chưa"), **độ phủ nguồn quy kết**, và xuất CSV đúng bảng đang xem. Người chỉ có quyền "xem của mình" chỉ thấy dòng khớp **email đăng nhập khai trong hồ sơ nhân sự** — không so tên. |
| **Cần xử lý & thông báo** | Quét mỗi 10 phút và ngay sau webhook: **đơn thiếu SĐT / địa chỉ** (báo ngay, không chờ quá hạn, kèm gợi ý lấy lại từ đơn cũ của khách), đơn chờ xử lý quá N giờ, vận đơn giao thất bại chờ phát lại, vận đơn treo không cập nhật N ngày, đang chuyển hoàn, case CSKH mới → chuông thông báo trên mọi trang, trang “Cần xử lý” lọc theo loại, gửi vào nhóm **Lark Suite** (Custom Bot webhook) và/hoặc Telegram; mỗi vấn đề báo một lần, tự đóng khi đã xử lý. |
...
| **Khách cũ mua lại · chống sót đơn** | Khách quen chốt lại qua chat thường không nhắn lại SĐT / địa chỉ ("gửi về địa chỉ cũ") nên đơn mới bị trống và không gửi được ĐVVC. Ba lớp chặn: (1) cảnh báo **Đơn thiếu SĐT / địa chỉ** ngay khi đơn chưa có vận đơn, ghi rõ thiếu gì; (2) trang đơn và nội dung cảnh báo hiện **gợi ý từ đơn cũ của chính khách** — khớp theo mã khách Pancake → hội thoại → SĐT nên nhận ra khách cũ kể cả khi đơn trống SĐT, kèm nút sao chép và link mở hội thoại; gợi ý chứ không tự điền vì khách có thể đã chuyển nhà; (3) case CSKH **"Đã chốt trong chat · chưa tạo đơn"** khi tin nhắn của shop có cụm chốt đơn (khai ở `cs.rules.closingKeywords`) mà hội thoại chưa sinh đơn mới, mở lại mỗi ngày cho tới khi có đơn. |
...
| **Chi phí vận hành** (`/expenses`) | Module kê khai chi phí vận hành kinh doanh theo nhóm (lương, mặt bằng, phần mềm, đóng gói, nhập hàng…), tách khỏi Quảng cáo. Sao kê ngân hàng **không** tạo chi phí từ giao diện: nhập ở Sổ ngân hàng (`/bank?tab=nhap-sao-ke`) → phân loại → nối chứng từ. Chỉ chủ shop chạy `npm run import:bank -- file.csv` / ops `import-bank-ledger` một cách tường minh (ghi `cost_source = BANK_IMPORT`, từ chối quảng cáo / tiền hàng / cước / phí hoàn), `bank-ledger-prune`. |
| **Tỷ lệ giao thành công theo mã hàng** | Chỉ số chính là tỷ lệ GIAO THÀNH CÔNG (không phải tỷ lệ hoàn, vì hoàn cần trạng thái Viettel Post mà API không tra được đơn tạo qua Pancake). Đơn giao thành công = đơn có **doanh thu COD thực > 100.000đ** (tiền thực thu theo webhook / bảng kê / danh sách vận đơn; chưa có số thực thu thì COD vận đơn khi VTP báo giao thành công; đơn chuyển khoản trước > 100K cũng tính) — không phụ thuộc trạng thái vận đơn. Không thành công = hoàn theo trạng thái hoặc giao nhưng COD ≤ 100.000đ (khách chỉ trả tiền ship / phí xem hàng). Tỷ lệ GTC = giao TC / (giao TC + không TC) trên đơn đã kết thúc, cột Dự kiến tính cả đơn chờ phát lại; cùng định nghĩa dùng cho Báo cáo lợi nhuận (TL GTC ước tính, giả định nhập theo GTC), Quảng cáo và Lương. Nhập **Danh sách vận đơn** xuất từ viettelpost.vn: file để mã vận đơn CHIỀU VỀ (mã gốc + "1P1") ở cột "Mã Vận Đơn" và mã gốc ở cột "Mã đơn hàng", nên ERP tra cả hai cột; dòng chiều về được ghi thành vận đơn riêng trỏ về mã gốc (không đè trạng thái đơn gốc) để quy tắc vận đơn chiều về tính đơn gốc là đơn hoàn. File Excel của viettelpost.vn khai báo sai vùng dữ liệu (`<dimension ref="A1:AU23"/>` cho sheet hàng nghìn dòng) — Excel bỏ qua nhưng thư viện đọc thì tin theo nên chỉ lấy 23 dòng đầu; ERP tính lại vùng dữ liệu từ các ô có thật để đọc đủ. Bấm mã xem danh sách đơn; xuất CSV. |
...
| **Quảng cáo** (`/ads`) — hiệu suất theo Marketer & mã hàng | Module riêng: KPI chi QC / đơn xác nhận / ROAS / CPO, bảng xếp hạng marketer và mã hàng theo chi QC, tin nhắn, đơn đã xác nhận (chia theo ảnh chụp fanpage theo mốc đơn → bảng gán phẳng → ad_id → tiền QC → chủ mã, phần không gắn được ở “Chưa gán marketer” nên tổng khớp đơn xác nhận), CPO, doanh số GTC ước tính, ROAS, LN sau QC = LN ròng ước tính (đã trừ mọi chi phí), tỷ lệ hoàn thực tế & dự kiến; kèm ngưỡng thanh toán tài khoản QC, biểu đồ chi tiêu, ghép chiến dịch → mã / marketer; đánh giá Tốt / Trung bình / Kém so với ROAS trung bình shop (cùng số liệu với Báo cáo lợi nhuận và Lương). |
...
| **Mua hàng & xưởng** | Tiền đang cam kết với xưởng (lô đã gửi chưa nhận), lô quá hạn hẹn, thời gian giao và tỷ lệ đúng hạn theo xưởng, giá nhập bình quân kỳ này so kỳ trước, và mẫu mã có giá nhập tăng từ 10% so với chính lần nhập trước. Đơn sản xuất KHÔNG có cột ngày nhận hàng nên thời gian giao là ƯỚC TÍNH: suy bằng cách ghép lô đặt với phiếu nhập kho và chỉ ghép khi một-một (cùng mẫu, cùng xưởng nếu cả hai bên đều khai, trong 180 ngày); lô có nhiều phiếu ứng viên là NHẬP NHẰNG, đếm riêng chứ không bốc đại. Trang chỉ đọc. |
| **Giữ chân khách** | Tỷ lệ mua lại tính trên ĐƠN GIAO THÀNH CÔNG (khách đặt nhiều đơn rồi hoàn hết không phải khách trung thành), hiện kèm con số đếm theo đơn đã đặt và khoảng chênh giữa hai cách; cohort giữ chân theo tháng nhận hàng lần đầu (tháng chưa tới để trống, không phải 0); phân khúc Mới / Đã mua lại / Trung thành / Nguy cơ / Đã rời bỏ kèm việc nên làm; trung vị số ngày tới lần mua thứ hai; danh sách khách nguy cơ; độ phủ gán khách là trần độ tin của cả trang. |
| **Mô phỏng kịch bản** | Nếu tỷ lệ giao thành công, giá bán, giá vốn, cước, chi quảng cáo hay chi vận hành đổi thì lợi nhuận đi về đâu — tính trên chính số của kỳ đang xem, cùng công thức với Báo cáo lợi nhuận. Kèm điểm hoà vốn theo tỷ lệ giao. Chi quảng cáo CHỈ là đòn bẩy chi phí (doanh thu không tăng theo ngân sách) vì độ phủ quy kết chưa đủ. Không lưu kịch bản, không ghi bất kỳ dữ liệu nào. |
...
- **Chi phí vận hành / Quảng cáo:** kê khai chi phí vận hành ở module Chi phí; chi tiêu QC tự kéo từ Facebook (nhập tay ở module Quảng cáo nếu cần) để báo cáo lợi nhuận đúng.
