...
| **SĐT mới · xác nhận số & xin số phụ** | Đơn chưa gửi ĐVVC (3 ngày gần nhất) mà SĐT chưa từng mua (Pancake tô xanh: khách chưa có đơn giao thành công / hoàn và ERP không có đơn khác cùng số) → bot nhắn khách qua Pancake xác nhận SĐT đúng chưa và xin số phụ (người thân). Đọc chat trước: shop đã hỏi → chờ khách; khách đã trả lời số / “đúng rồi” → đóng case. Đơn không có hội thoại (landing page / sheet) → case ⛔ để gọi tay. Banner xanh trên chi tiết đơn; mẫu tin `phoneVerifyTemplate`, bật/tắt `phoneVerifyAuto` trong `cs.rules`; job `phone-verify`. |
