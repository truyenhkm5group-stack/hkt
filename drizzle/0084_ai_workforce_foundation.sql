...
-- chúng đã chạy ở 0033 và 0034 rồi, chỉ xuất hiện lại vì ảnh chụp lược đồ trong kho bị lệch.
--
-- ĐÁNH SỐ LẠI 14/09/2026: tệp này từng là `0035_ai_workforce_foundation`. Khi nhánh nhân sự AI
-- nhập `main` về, số 0035 và 0036 đã thuộc về `manual_verification_source` và
-- `expense_cost_allocation` — hai migration ĐÃ CHẠY trên production. Giữ nguyên số cũ thì drizzle
-- (vốn chạy theo `_journal.json`) sẽ thấy mục 0035 đã áp và BỎ QUA VĨNH VIỄN tệp này: không lỗi,
-- không cảnh báo, bảng đơn giản là không bao giờ tồn tại. Đó đúng là sự cố 12/09/2026 mà
-- `tests/migration-upgrade-path.test.ts` sinh ra để chặn.
--
-- Ảnh chụp lược đồ (`drizzle/meta/*_snapshot.json`) của hai migration này đã được GỠ BỎ chứ không
-- đánh số lại: chúng mô tả thế giới ở thời điểm 0032 cộng thêm các bảng AI, nên để lại làm ảnh
-- chụp MỚI NHẤT thì lần `db:generate` sau sẽ tưởng ~50 bảng của main chưa tồn tại và sinh lệnh
-- tạo lại. Kho mã này vốn đã dừng cập nhật ảnh chụp từ 0032 (main thêm 51 migration mà không có
-- ảnh chụp nào) — gỡ đi là giữ nguyên hiện trạng đó, không tạo thêm một cái bẫy mới.
