...
--
-- Viết tay và idempotent (`IF NOT EXISTS`) như 0033–0066: ảnh chụp `drizzle/meta/*_snapshot.json`
...
-- 'migration:0067' — thà nói thẳng "không biết ai" còn hơn gán bừa cho một người thật.
...
SELECT md5("id" || ':' || "linked_type" || ':' || "linked_id"), "id", "linked_type", "linked_id", abs("amount"),
       'MANUAL', 'MANUAL',
       CASE WHEN length(trim(coalesce("classified_by", ''))) > 0 THEN "classified_by" ELSE 'migration:0067' END,
       coalesce("classified_at", "created_at"),
       'Chuyển từ linked_type/linked_id khi lên bảng nối nhiều–nhiều (0067)'
