...
/*
 * KIỂM KÊ ĐỘ ĐẦY ĐỦ ĐÃ CHUYỂN SANG `lib/queries/sales-knowledge.ts`.
 *
 * `auditWinOffer()` từng sống ở đây và tính lại chính những gì cổng năng lực tính — hai đường đọc
 * cho cùng một câu hỏi "hồ sơ này còn thiếu gì", và chúng đã bắt đầu trả lời khác nhau (bản ở đây
 * không biết bảng số đo nằm ở máy gợi ý size, cũng không biết quyền nào đang chặn). Dùng
 * `loadWinKnowledge()` — nó trả về năng lực, dữ liệu thiếu, quyền bị chặn và xung đột với ERP
 * trong một lượt đọc.
 */
