...
async function build(period: Period): Promise<CashflowStatement> {
  ...
  */
  ...
  /**
   * TIỀN VÀO / RA KINH DOANH = KHOANG `OPERATING`, KHÔNG PHẢI "mọi khoang trừ EXCLUDED".
   *
   * Hai lý do, và lý do thứ hai mới là lý do thật:
   *
   *  1. Chuyển 10 triệu giữa hai tài khoản của mình mà tính vào thì "tiền vào" và "tiền ra" mỗi bên
   *     cộng thêm 10 triệu: chênh lệch vẫn đúng nhưng cả hai con số tổng đều bị thổi phồng.
   *  2. **Vốn góp và tiền vay KHÔNG phải tiền kinh doanh.** Bản đầu loại mỗi khoang `EXCLUDED`, nên
   *     `FINANCING` lọt vào: chủ shop bơm 100 triệu và ngân hàng giải ngân 50 triệu thì "tiền vào
   *     kinh doanh" của kỳ nhảy thêm 150 triệu. Đó đúng là tình huống mà `CASHFLOW_SECTION_HINT`
   *     ngay trên đây cảnh báo — "một shop đang lỗ mà đi vay trông như một shop đang bán tốt" — và
   *     `isBusinessCash()` ở `lib/constants/bank.ts` cũng loại `CAPITAL`/`OWNER` từ đầu. Con số
   *     mang nhãn "kinh doanh" mà gồm cả tiền vay thì nhãn đó là sai, không phải cách đọc khác.
   *
   * `INVESTING` và `FINANCING` KHÔNG biến mất: chúng vẫn là khoang riêng trên báo cáo, và
   * `movementAll` vẫn cộng đủ mọi dòng cho phép kiểm đẳng thức số dư. Chỉ con số HEADLINE mới thu
   * về đúng phần nói lên shop có tự nuôi được mình hay không.
   *
   * `tests/finance-invariants.test.ts` nhóm 11 khoá con số này bằng đúng con số của
   * `getCashLedger()` — hai phép cộng khác nhau cho một câu hỏi phải ra một kết quả.
   */
  const business = sections.filter((s) => s.section === "OPERATING");
  ...
  const prevBusiness = prevRows
    ...
    // Kỳ trước phải đo CÙNG MỘT THỨ với kỳ này, nếu không phép so kỳ là so hai định nghĩa.
    .filter((r) => sectionOf(r.group) === "OPERATING");
