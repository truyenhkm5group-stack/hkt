...
export function orderListWhere(params: ListParams, opts: { boQuaLocDiaChi?: boolean } = {}) {
  const conds: (SQL | undefined)[] = [];
  const { period, q } = params;
  const filters: Record<string, string[] | undefined> = opts.boQuaLocDiaChi ? { ...params.filters, address: undefined } : params.filters;
...
export async function orderSummary(params: ListParams) {
  ...
  const [chuaChuanHoa] = await db
    .select({ n: count() })
    .from(schema.orders)
    .where(and(orderListWhere(params, { boQuaLocDiaChi: true }), sql`coalesce(${schema.orders.shipProvince}, '') = ''`, sql`${schema.orders.stage} not in ('CANCELLED','DELETED')`));
  return { orders: Number(row?.orders ?? 0), revenue: Number(row?.revenue ?? 0), cod: Number(row?.cod ?? 0), success: Number(row?.success ?? 0), quantity: Number(row?.quantity ?? 0), unnormalizedAddress: Number(chuaChuanHoa?.n ?? 0) };
}
