...
import { PAYROLL_EMPLOYEES_KEY, type Employee, type PayrollBasis } from "@/lib/constants/payroll";
import { getCashProfitReport } from "@/lib/queries/profit-cash";
import { getNominalProfitReport, type NominalReport } from "@/lib/queries/profit-nominal";
import type { Period } from "@/lib/search-params";
import { getSettingJson } from "@/lib/settings";

export async function listEmployees(): Promise<Employee[]> {
  const list = await getSettingJson<{ list: Employee[] }>(PAYROLL_EMPLOYEES_KEY, { list: [] });
  const defaults = (): Omit<Employee, "id" | "name"> => ({ aliases: [], accountIds: [], fixed: 0, percentTotal: 0, percentPersonal: 0, percentRevenue: 0, active: true, note: "", department: "Marketing", shortName: "" });
  return (list.list ?? []).map((e) => ({ ...defaults(), ...(e as Partial<Employee>) }) as Employee);
}

export type MarketerProductLine = { productId: string; productName: string; code: string; adSpend: number; share: number; attributedRevenue: number; attributedProfitBeforeAds: number; personalProfit: number; orders: number };
 ...
 */
export async function getMarketerReport(period: Period): Promise<MarketerReport> {
  const db = await getDb();
  const [nominal, employees] = await Promise.all([getNominalProfitReport(period), listEmployees()]);
  ...
  const ensure = (id: string | null) => {
    ...
    if (!m) {
      ...
      m = { marketerId: id, name: emp ? emp.shortName || emp.name : "Chưa gán marketer", adSpend: 0, testSpend: 0, totalSpend: 0, attributedRevenue: 0, attributedOrders: 0, attributedProfitBeforeAds: 0, personalProfit: 0, products: [] };
      marketers.set(id, m);
    }
    return m;
  };
  ...
  for (const [marketerId, amount] of testByMarketer) ensure(marketerId).testSpend += amount;
...
export type PayrollReport = { basis: PayrollBasis; totalProfit: number; lines: PayrollLine[]; totalSalary: number; marketers: MarketerReport };

/** Bảng lương theo kỳ: lương cứng + % lợi nhuận tổng + % lợi nhuận cá nhân + % doanh thu cá nhân (thưởng chỉ tính khi số dương) */
export async function getPayrollReport(period: Period, basis: PayrollBasis): Promise<PayrollReport> {
  const [marketers, employees, cash] = await Promise.all([getMarketerReport(period), listEmployees(), basis === "cash" ? getCashProfitReport(period) : Promise.resolve(null)]);
  const totalProfit = basis === "cash" && cash ? cash.net : marketers.nominal.totals.expectedProfit;
  const lines: PayrollLine[] = employees
    ...
    .map((e) => {
      ...
      const bonusTotal = Math.round(Math.max(totalProfit, 0) * (e.percentTotal / 100));
      const bonusPersonal = Math.round(Math.max(personalProfit ?? 0, 0) * (e.percentPersonal / 100));
      const bonusRevenue = Math.round(Math.max(personalRevenue ?? 0, 0) * (e.percentRevenue / 100));
      return { employee: e, totalProfit, personalProfit, personalRevenue, fixed: e.fixed, bonusTotal, bonusPersonal, bonusRevenue, salary: e.fixed + bonusTotal + bonusPersonal + bonusRevenue };
    });
  return { basis, totalProfit, lines, totalSalary: lines.reduce((s, l) => s + l.salary, 0), marketers };
}
...
export async function unassignedMarketerSpend(period: Period) {
  ...
  if (period.from) conds.push(gte(ads.spendDate, period.from));
  if (period.to) conds.push(lte(ads.spendDate, period.to));
  const [row] = await db.select({ spend: sql<number>`coalesce(sum(${ads.spend}), 0)`, campaigns: sql<number>`count(distinct ${ads.campaignId})` }).from(ads).where(and(...conds));
  return { spend: Number(row?.spend ?? 0), campaigns: Number(row?.campaigns ?? 0) };
}
