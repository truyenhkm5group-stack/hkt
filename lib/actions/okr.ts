...
import { isMetricKey, METRIC_UNITS } from "@/lib/constants/metric-bindings";
  ...
  if (d.useTemplate) {
    ...
    if (tpl?.length) {
      await db.insert(schema.bscMetrics).values(
        tpl.map((m, i) => ({ scorecardId: id, perspective: m.perspective, label: m.label, metricSource: m.metricSource, weight: m.weight, sortOrder: (i + 1) * 10, unit: "NUMBER" as const, direction: "UP" as const })),
      );
    }
  }
