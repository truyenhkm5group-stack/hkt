...
## 13. PHÁT HÀNH RIÊNG PERFORMANCE P0 — 09/09/2026

### 13.1 Vì sao tách riêng, và tách bằng cách nào

Cây thư mục làm việc lúc phát hành có **năm phiên khác** đang sửa cùng lúc (nhận diện quảng cáo, mua
hàng & xưởng, sổ ngân hàng, giữ chân khách, phân quyền API). `git commit` từ cây đó sẽ gom cả phần
việc đang dở của họ vào một commit mang tên "hiệu năng".

Cách làm: dựng một **worktree sạch** tại đúng commit production đang chạy (`c725878`), rồi đưa SANG
đó chỉ phần Performance P0 — bằng bản vá cho tệp đã theo dõi, cộng bản sao cho tệp mới. Không dùng
`git add .`, không `git reset --hard`, không `checkout` đè lên cây chung.

Nền chọn là commit **production đang chạy**, không phải `main`, vì `main` đang chứa bốn commit tính
năng của phiên khác chưa được kiểm chứng để phát hành. Nhờ vậy bản phát hành này đúng nghĩa "chỉ
Performance P0": không kéo theo tính năng nào khác.

Ba lần suýt lẫn — đều bị chặn lại bằng cách đối chiếu từng tệp:

| Suýt lẫn | Chặn bằng |
|---|---|
| Bộ lọc "bộ phận" của hàng đợi việc lọt vào `alerts/page.tsx` + `queue-filters.tsx` | `tsc` báo thiếu `TEAM_LABEL` ⇒ trả hai tệp về gốc rồi áp lại đúng phần P0 |
| Mục menu "Sổ ngân hàng", `operatingExpenseCond()`, `EXPENSE_CATEGORIES_NOT_OWNED` lọt vào `app-sidebar` / `dashboard.ts` / `reports.ts` | gỡ đúng những dòng đó, giữ nguyên phần P0 |
| **Ba tệp P0 bị BỎ SÓT** (`reports.ts`, `dashboard.ts`, `app-sidebar.tsx`) — danh sách tệp lấy lúc cây đang bị phiên khác ghi | quét lại toàn kho theo DẤU VĂN của P0 (`useNavTransition`, `OUTCOME_FENCE`, `listKey(`…) thay vì tin vào một lần chụp `git status` |

`main` **đang hỏng** ở thời điểm này: `tests/sync-fixtures.test.ts` (do phiên khác commit) `import`
tệp `tests/metric-shape-consistency.test.ts` chưa hề được commit, nên `npm test` đứt ngay ở bước nạp
mô-đun — tức cổng deploy của MỌI người đang bị chặn. Bản phát hành này mang theo tệp đó nên gỡ luôn
nút thắt ấy.

### 13.2 Số liệu cổng phát hành (nền = commit production `c725878`)

Đo lại từ đầu trên bản ứng viên sạch, cùng bộ công cụ, cùng máy:

| TRANG | TRƯỚC @1× | SAU @1× | LỢI | TRƯỚC @4× | SAU @4× | LỢI | ẤM (đệm) | SỐ CÂU |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| Sản phẩm & tồn kho (Products) | 2.155 ms | 207 ms | **10.4×** | 24.330 ms | 815 ms | **29.8×** | 61 ms | 9 |
| Tổng quan (Dashboard) | 2.012 ms | 389 ms | **5.2×** | 20.750 ms | 1.137 ms | **18.2×** | 0 ms | 55 |
| Chất lượng dữ liệu (Data Quality) | 881 ms | 334 ms | **2.6×** | 9.384 ms | 1.388 ms | **6.8×** | 0 ms | 29 |
| Quảng cáo (Ads) | 800 ms | 315 ms | **2.5×** | 7.531 ms | 933 ms | **8.1×** | 4 ms | 31 |
| Báo cáo lợi nhuận (Profit) | 233 ms | 106 ms | **2.2×** | 1.840 ms | 298 ms | **6.2×** | 0 ms | 17 |
| Vận đơn (Shipments) | 227 ms | 67 ms | **3.4×** | 2.523 ms | 188 ms | **13.4×** | 13 ms | 9 |
| Hiệu quả mẫu mã (Product) | 195 ms | 20 ms | **9.6×** | 1.621 ms | 44 ms | **36.8×** | 0 ms | 1 |
| GTC theo mẫu mã | 162 ms | 28 ms | **5.9×** | 1.516 ms | 66 ms | **22.9×** | 16 ms | 2 |
| Đơn hàng (Orders) | 114 ms | 33 ms | **3.4×** | 1.126 ms | 101 ms | **11.1×** | 4 ms | 7 |
| Chân lý tài chính (Truth) | 94 ms | 55 ms | **1.7×** | 985 ms | 129 ms | **7.7×** | 0 ms | 5 |
| Tỷ lệ giao thành công (GTC) | 70 ms | 23 ms | **3.0×** | 630 ms | 62 ms | **10.2×** | 10 ms | 2 |
| Trung tâm điều khiển (22 luật) | 27 ms | 30 ms | **0.9×** | 59 ms | 60 ms | **1.0×** | 0 ms | 25 |
| Hàng đợi việc (Action Queue) | 7 ms | 9 ms | **0.8×** | 13 ms | 10 ms | **1.4×** | 8 ms | 3 |
| Tồn kho (Inventory) | 3 ms | 3 ms | **1.0×** | 3 ms | 3 ms | **0.9×** | 2 ms | 6 |

Cả năm trang mà yêu cầu nêu đích danh đều đạt: Tổng quan **5,2×**, Chất lượng dữ liệu **2,6×**,
Quảng cáo **2,5×**, Hiệu quả mẫu mã **9,6×**, Báo cáo lợi nhuận **2,2×** — và ở quy mô gấp 4 thì
6,2×–36,8×.

### 13.3 Rà soát thay đổi nghiệp vụ đi kèm (ORDER_OUTCOME ↔ SHIPMENT_LEFT_WAREHOUSE)

`scripts/bench/stock-rule-verify.ts` dựng tám tình huống phủ đúng năm chiều mà yêu cầu đòi đối
chiếu — đã xuất kho · rời kho theo ĐVVC · giao thành công · hoàn · huỷ:

| Mẫu mã | Tình huống | Tồn (công thức CŨ) | Tồn (công thức MỚI) | Kết quả đơn |
|---|---|---:|---:|---|
| v1 | Đang giao, đã lấy hàng | 9 | 9 | IN_TRANSIT |
| v2 | Giao thành công, thu đủ tiền | 9 | 9 | DELIVERED |
| v3 | Hoàn, kho CHƯA nhận | 9 | 9 | RETURNED |
| **v4** | **Hoàn, kho ĐÃ nhận và đã lập phiếu tái nhập** | **11** | **10** | RETURNED |
| **v5** | **Huỷ SAU khi đã xuất kho** | **10** | **9** | CANCELLED |
| v6 | Đã tạo vận đơn, bưu tá chưa lấy | 10 | 10 | UNKNOWN |
| v7 | Có vận đơn nhưng không dấu vết ĐVVC | 10 | 10 | UNKNOWN |
| v8 | Tiêu huỷ 503 — hàng không quay về kho | 9 | 9 | RETURNED |

Hai chỗ lệch, và **cả hai đều là công thức cũ khai KHỐNG tồn kho**:

* **v4** — kiện hoàn đã được kho đếm và nhập lại. Công thức cũ không trừ chiều đi (vì đơn không còn
  ở trạng thái "chờ kho nhận"), nhưng phiếu tái nhập vẫn cộng vào ⇒ **một mẫu mã nhập 10 cái thành
  11**. Đúng cái nghiệp vụ mà phiên "kiểm đếm hàng hoàn" vừa làm cho phổ biến hơn.
* **v5** — đơn huỷ sau khi hàng đã rời kho. Công thức cũ coi như hàng chưa bao giờ đi, trong khi
  `HANDOFF.md` mục 6.7 ghi rõ: "Đơn huỷ sau khi đã xuất xử lý y như hàng hoàn".

Các KPI **không đổi**, và không thể đổi: `ERP_STOCK_SUB` chỉ được dùng ở **6 chỗ, tất cả trong
`lib/queries/products.ts`** (3 nhánh bộ lọc tồn + 3 bộ đếm facet). Không một báo cáo tiền, GTC, COD
hay lợi nhuận nào chạm tới nó. Số đo trong cùng lần chạy: giao TC 1 · hoàn 3 · huỷ 1 · đang giao 1 ·
chưa biết 2 · GTC 25,0% · rời kho 6 đơn — tính bằng `ORDER_OUTCOME`, độc lập hoàn toàn.

**Kết luận: GIỮ trong bản phát hành này.** Nó vừa là điều kiện của phần tăng tốc (bỏ `ORDER_OUTCOME`
khỏi một biểu thức chạy cho từng mẫu mã × từng cột gộp: 2.155 ms → 207 ms), vừa có bằng chứng nghiệp
vụ ở trên, vừa đúng quy tắc đã ghi thành văn trong `AGENTS.md` mục 3.10. Khoá bằng assertion mới
trong `tests/metric-shape-consistency.test.ts`: ba bộ đếm bộ lọc phải bằng số dòng đếm được từ chính
cột tồn của bảng.

### 13.4 Rà soát migration 0041

| Điều kiểm | Kết quả |
|---|---|
| Mã migration duy nhất (`idx`, `tag`) | Đạt — `idx 41` chưa ai dùng, `tag` chưa có |
| Production đã áp chưa | **Chưa** — chưa từng nằm trên `main`; production đang ở `c725878`, mốc cuối `1788839392150` |
| Mốc `when` không bị bỏ qua | `1788940601682` **lớn hơn** mốc cuối ⇒ chắc chắn được áp (đúng bài học của lỗi 0038) |
| Mọi mục trong sổ đều có tệp `.sql` và ngược lại | Đạt |
| Câu lệnh phù hợp | `CREATE INDEX IF NOT EXISTS … WHERE order_reference IS NOT NULL` — index riêng phần, chỉ phủ vận đơn có mã tham chiếu |
| Không phá huỷ | Đạt — không `DROP`/`ALTER`/`DELETE`/`UPDATE`, chỉ một câu `CREATE INDEX` |
| Khoá bảng bao lâu | `CREATE INDEX` thường (không `CONCURRENTLY`) giữ khoá ghi trên `shipments`. Production có **1.773 vận đơn** ⇒ dưới một giây. Cố ý không dùng `CONCURRENTLY`: drizzle chạy migration TRONG một transaction, mà `CONCURRENTLY` không chạy được trong transaction |
| Chạy lại có an toàn | Đạt — `IF NOT EXISTS`, đã thử chạy hai lần |
| Quay lui | `DROP INDEX IF EXISTS "shipments_order_reference_lookup_idx";` — không mất dữ liệu, chỉ chậm lại như cũ |
| Index có được DÙNG thật không | **Có** — `EXPLAIN ANALYZE` cho `Index Scan using shipments_order_reference_lookup_idx on shipments leg` |

Diễn tập bằng `scripts/bench/migration-verify.ts`: áp từ CSDL trống hết **2.542 ms**, 42 migration,
index tồn tại đúng định nghĩa, chạy lại không lỗi.

### 13.5 Kiểm chứng trải nghiệm chờ

Hai lớp, vì có hai loại câu hỏi khác nhau.

**a) `tests/loading-ux-contract.test.ts` (mới) — khoá phần dây nối ở mức mã nguồn**, chạy trong
`npm test` nên cổng deploy giữ luôn:

* 21 trang nặng đều có `loading.tsx` riêng và đều dựng khung xương (không trả `null`);
* `NavProgressProvider` + `StaleWhileRefreshing` mắc trong layout;
* ngưỡng chống nháy 180 ms áp cho **cả ba** thứ hiện khi chờ (thanh tiến trình, nhãn, lớp mờ);
* thanh lọc truyền `startTransition` cho nuqs — điều kiện để React giữ số cũ và để **lần điều hướng
  mới nhất luôn thắng**;
* ô ngày tuỳ chọn chống rung ≥ 300 ms và chỉ gửi khi ngày hợp lệ;
* **quét toàn kho**: không còn component nào gây điều hướng mà dùng `React.useTransition` trần.
  Chính bài kiểm thử này bắt được hai chỗ còn sót — tab vai trò của Phễu bán hàng và chuông thông
  báo — đã sửa. Ba ngoại lệ còn lại (transition bọc thao tác GHI / TẢI, không bọc điều hướng) được
  ghi tên kèm lý do, và bài kiểm thử bắt buộc các tệp đó phải còn tồn tại;
* `error.tsx` có nút thử lại · `EmptyState` riêng · ba hình khung xương riêng ⇒ tải / trống / lỗi
  không thể nhìn giống nhau.

**b) `scripts/bench/ux-verify.ts` (mới) — đo hành vi thật trên bản production build đang chạy:**

| Kịch bản | Kết quả |
|---|---|
| Bấm 10 tab liên tiếp | 10/10 HTTP 200, chậm nhất **119 ms** |
| Phản hồi nhanh không nháy spinner | **10/10** tab dưới ngưỡng 180 ms ⇒ không hiện gì cả |
| Đổi kỳ liên tục (5 yêu cầu chồng nhau) | cả 5 trả về **đúng kỳ của chính nó**, không lẫn sang kỳ khác |
| Quay lại kỳ vừa xem | 116 ms → 109 ms (đệm ăn) |
| Đường API lỗi | `401` kèm thông báo · trang không tồn tại `404` — khác hẳn màn hình trống |

**Chưa kiểm được bằng máy, cần người bấm thử:** thanh tiến trình / khung xương / lớp mờ có hiện đúng
lúc bằng MẮT hay không, và hành vi trên mạng chậm thật. Kho mã không có công cụ điều khiển trình
duyệt, và cài thêm một bộ như vậy ngay trước lúc phát hành là thêm rủi ro mà không đổi lấy gì. Đây
chính là việc chủ shop làm khi mở ERP sau khi deploy.

*Lưu ý về con số ở kịch bản "đổi kỳ liên tục": mỗi yêu cầu mất khoảng 1,66 giây vì năm yêu cầu chạy
CHỒNG NHAU trên PGlite — vốn chỉ có MỘT kết nối nên chúng phải xếp hàng. Postgres 16 trên VPS dùng
pool nhiều kết nối nên không có nút thắt này. Điều kịch bản đó kiểm là TÍNH ĐÚNG (không lẫn kỳ),
không phải tốc độ.*
