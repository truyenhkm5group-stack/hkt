...
| `confirm-funnel-audit` *(thêm 25/09/2026)* | không in dữ liệu cá nhân — cố ý KHÔNG tách theo nhân viên (tên nhân viên chỉ xem trên trang có đăng nhập); bọc mã hoá để phần in thêm (nếu có) không ra log | tóm tắt: số "đã xác nhận" theo định nghĩa cũ và mới, trung vị thời gian xác nhận cũ / mới, đơn huỷ trước / sau xác nhận và huỷ sau bao lâu, theo nguồn đơn, theo giờ lên đơn. `--gio`: đếm theo giờ / ngày / PHÚT (gộp, ≥ 5 đơn) và số đơn thiếu mốc Pancake. `--xoa`: đếm Đã xoá vs Đã huỷ theo từng dấu hiệu. Không mã đơn, không SĐT |
| `company-os-summary` *(thêm 26/09/2026)* | không in dữ liệu cá nhân — không đọc cột tên / SĐT / email / địa chỉ / tiêu đề / ghi chú / mã (bài kiểm quét SQL); mã mẫu mơ hồ chỉ được ĐẾM; bọc mã hoá để phần in thêm (nếu có) không ra log | tóm tắt: mức dùng Company OS — mẫu theo vòng đời, sổ mẫu xem trước, job model-registry / dashboard-warm, sự kiện theo tên, topic / giá thành / mẫu xưởng / bản thiết kế / lệnh SX trỏ thiết kế, phản ứng "Cần anh quyết", tin Lark bật/tắt, kết cục hàng hoàn, yêu cầu duyệt, phiếu nhập gắn lệnh / lô. Mục không đọc được in "—" kèm lý do |
