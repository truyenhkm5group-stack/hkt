...
Nhánh `claude/cos-h-cockpit` (dựng trên A · B · C · D · E · F · G · A2) · migration `0138_company_os_cockpit`
...
| Bảng `recommendation_decisions` (append-only, CHECK loại / quyết định / lý do bỏ qua / ngày nhắc, `decided_by_user_id` NOT NULL FK RESTRICT, `snapshot` jsonb) | `db/schema.ts` (khối cuối), `drizzle/0138_company_os_cockpit.sql`, `drizzle/meta/_journal.json` |
