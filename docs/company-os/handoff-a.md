...
Nhánh `claude/cos-a-model-foundation` · migration `0131_company_os_models` · hợp đồng `docs/company-os/shared-contracts.md` mục 1–2 (nằm ở nhánh Tech Lead).

## Đã dựng

| Phần | Tệp |
|---|---|
| Lược đồ: `product_models`, `domain_events`, `product_model_state_history` (khối cuối `db/schema.ts`) | `db/schema.ts`, `drizzle/0131_company_os_models.sql`, `drizzle/meta/_journal.json` |
