...
Nhánh `claude/cos-d-inventory` (từ `origin/main` 5a3a7ee6) · migration `0132_company_os_inventory`.
...
| 3 | Cột `stock_receipts.production_order_id` / `production_batch_id` (NULL, FK ON DELETE SET NULL, chỉ mục một phần), không backfill. Hai ô chọn trên form NHẬP HÀNG (lệnh `SENT`, lô `OPEN`; đã gõ tên xưởng ⇒ chỉ hiện của xưởng đó). Máy chủ kiểm: chỉ phiếu `RECEIPT`, mã tồn tại, không huỷ, lô không nối với lệnh KHÁC. Hiện liên kết ở danh sách + chi tiết phiếu. | `db/schema.ts`, `drizzle/0132_company_os_inventory.sql`, `lib/inventory/production-link.ts`, `lib/queries/stock.ts::listOpenProductionLinks`, `receipt-dialog.tsx`, `receipts/page.tsx` |
