...
Nhánh `claude/cos-f-economics` (từ `origin/main` 5a3a7ee6). Migration `0133_company_os_economics`.
...
`lib/queries/production-variance.ts`, `drizzle/0133_company_os_economics.sql`,
...
`model-economics`. `migration-upgrade-path`: dòng sổ gieo trước 0133 giữ NULL cả ba cột.
...
migration mới — đã chuyển sang kiểm "không DEFAULT + tệp 0133 không có UPDATE".
...
- Chưa có màn hình đo SAI SỐ DỰ BÁO (so `projected_profit_after_ads` của ngày D với số đo khi cohort
  chín) — cần vài tuần dữ liệu sau khi deploy 0133 mới có gì để đo.
...
- Đánh số: `0133` với `idx 133` nhảy qua 131/132 (của A, D). Tech Lead đánh lại số/`when` lúc gộp
  nếu cần; `when = 1790005588245`.

## HUMAN GATE

- Deploy migration 0133 (thuần bổ sung, NULLABLE) — cần lượt deploy thường.
